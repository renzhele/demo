...
Analyze the existing `extract_action_items()` function in `app/services/extract.py`, which currently extracts action items using predefined heuristics.
...
Write unit tests for `extract_action_items_llm()` covering multiple inputs (e.g., bullet lists, keyword-prefixed lines, empty input) in `tests/test_extract.py`.
...
## Deliverables
Fill out `writeup.md` according to the instructions provided. Make sure all your changes are documented in your codebase. 
